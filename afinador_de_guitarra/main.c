...
// Enumeración para representar las notas musicales que se afinan
typedef enum {
    E2, // 82 Hz
    A, // 110 Hz
    D, // 147 Hz
...
// Variables globales
float factor = 0.000004;          // Factor de conversión de cuentas a segundos (asume 250 kHz SMCLK)
...
int main(void)
{
    ...
    P4DIR = 0x01;                 // Pin para salida de observación en osciloscopio
    P4OUT = 0x00;
    ...
    while(1){
        if (nueva_medida) {
            ...
            // Detectar la nota más cercana
            TiposNotas nota = detectarNota(frecuencia);

            // Mostrar la nota en display
            mostrar_nota(nota);

            // Indicar si la afinación está baja, precisa o alta
...
void confg_relog(void){
    TA0CCTL0 = CCIE;                         // Habilitar interrupción CCR0
    TA0CCR0 = 1000 - 1;                      // Cuenta hasta 999
    TA0CTL = TASSEL_2 | MC_1 | ID_0 | TACLR; // SMCLK, modo up, sin división
}
...
TiposNotas detectarNota(unsigned int f) {
    ...
    // Inicializa la variable "menor" con la primera diferencia para comparar después
